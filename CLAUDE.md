...
4. **Stream** (`feed_posts`, kinds `task | announcement | poll | assignment`) — approved tasks appear automatically unless the student turned `share_to_feed` off; advisor posts announcements (≤1 photo, ≤1 document, ≤1 link) and polls, to one or many groups, now or as drafts. Likes, flat comments, advisor moderation. Read through `list_feed_posts` (privacy boundary: never the reflection or the mentor's note).
