...
| 2026-09-24 → 2026-09-25 | Session 29 | **Review and approval move to the advisor.** Spec `docs/superpowers/specs/2026-09-24-advisor-review-design.md` (amends `2026-08-20-task-assignment-design.md`, which now carries a pointer to this one), plan under the same date, five tasks. The owners want the same person who assigns a task to judge it, and the simulation had already shown the cost of the old rule (a slow mentor left a submission pending until closure; a student with no mentor linked had nobody to review them). New file `docs/review-by-advisor.sql` becomes the home of `submit_assignment` and `review_assignment` (they lived in `docs/internship-closure-guards.sql`, which now carries the note that re-running it would put the mentor gate back): `review_assignment`'s role gate is `is_group_advisor_of` instead of `is_mentor_of`, still refusing with `ROLE_NOT_ALLOWED`; `submit_assignment`'s "Task Submitted" notification goes to the group's advisor instead of `student_profiles.mentor_id`. Everything else is unchanged — the closure guard, `ALREADY_APPROVED` on a second approval, the KPI observation, the in-transaction notification. The mentor keeps attendance confirmation (`internship_review`), messaging, and read-only access to the submission and the advisor's decision (`assignment_submissions` already admitted `is_mentor_of`, so no policy changed); history stands (the simulation's 24 mentor approvals are still valid observations from "someone other than the student"). Verification `docs/review-by-advisor-verification.sql`, run live by the owner: Part A 5/5 (both functions `SECURITY DEFINER`, fixed `search_path`, callable by `authenticated` and not `anon`); Part B 6/6 (advisor approval sets `approved` and level 2, `kpi_observations.observed_by` is the advisor, **the mentor is refused with `ROLE_NOT_ALLOWED`**, a second approval still raises `ALREADY_APPROVED`, `task_submitted` notifies the advisor and not the mentor, a closed internship still refuses); Part C 2/2 (the mentor still SELECTs the submission, their UPDATE affects 0 rows). Client: `pending-reviews.tsx` and `review-detail.tsx` moved from `app/(mentor)/` to `app/(advisor)/`, reusing `ReviewUI` / `ReviewNoteSheet` / `ReviewEvidence` / `AssignmentReview` / `mentorReviewStore` as they are — only the route, the tab and the store's owner changed; the advisor's now-multi-group queue gained the group chip the messages screen already used; the advisor dashboard gained the "next review" featured card the mentor home used to show, and the mentor dashboard's featured card became attendance (days waiting for confirmation) with the review tab gone. Copy: the student's task detail now reads "Advisor note" and "Your advisor will review your work after you submit" in place of the mentor's name (`studentFlow.mentorNote`, `studentFlow.reviewHint`, `student.whatIDidTodayHint` — en and tr only, the other five fall back), and the reviewer's own "Add a note" / "Previous note" (`mentorFlow.optionalNote`, `mentorFlow.previousNote`) lost the word "mentor" in all seven locales since `mentorFlow` is parity-tested; the JSON keys and the SQL columns (`mentor_note`, `mentor_level`, `competency_self_vs_mentor`) keep their historical names on purpose — renaming them would touch every reader for no behavioural gain. `CLAUDE.md` updated throughout (Project Overview, Domain Model item 3, Route Groups) to say the advisor reviews and the mentor's job is now attendance, messaging and read-only follow-up. Jest 70 suites / 647 tests, `tsc` silent, `eslint` clean. Commits `cedfe65` (spec) … `0d6fd27` (student copy); this row's commit closes the plan. Out of scope, left for the owners: whether the mentor should get a workplace note of their own on the submission. |
