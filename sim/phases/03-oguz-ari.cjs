// Phase 3 — Oğuz Arı links his student, Deniz Yıldırım. Adapted from the
// template (sim/phases/03-hakan-demir.cjs); no per-person extras for this
// slug in the brief (Step 2 extras are Ayça, Emre, Selin Kurt only).
//
// Adaptations carried over from the template (see task-6a-report.md):
...
//     PostgREST returns an ARRAY of rows, not a single object.
//  3. 'ABC123' is 6 characters, so the made-up-code refusal is INVALID_CODE
//     (no matching student_codes row), not INVALID_CODE_FORMAT.
...
(async () => {
  ...
  if (!(contacts || []).some((c) => c.id === student.userId)) me.bug({ did: 'read mentor message contacts after linking', expected: 'the advisor and my linked student', got: JSON.stringify(contacts), severity: 'wrong' });
  else me.note(`list_mentor_message_contacts → ${(contacts || []).length} contact(s); Deniz is listed.`);
