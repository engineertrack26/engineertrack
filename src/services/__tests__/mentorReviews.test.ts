...
describe('mentor review operations', () => {
  ...
  it('does not call the server for a blank revision reason', async () => {
    await expect(mentorReviewService.submit(item, false, '  ')).rejects.toThrow('mentorFlow.reasonRequired');
    expect(list).not.toHaveBeenCalled();
    expect(review).not.toHaveBeenCalled();
  });
  it('rechecks the specific submission and sends the trimmed reason', async () => {
    await mentorReviewService.submit(item, false, '  Add units  ');
  ...
  it('allows approval without a note', async () => {
    await mentorReviewService.submit(item, true, '');
  ...
  it('rejects a submission that was already reviewed or left the mentor scope', async () => {
    ...
    await expect(mentorReviewService.submit(item, true, '')).rejects.toThrow('mentorFlow.reviewChanged');
    expect(review).not.toHaveBeenCalled();
  });
  ...
  it('never notifies the student when the review fails', async () => {
    ...
    await expect(mentorReviewService.submit(item, true, '')).rejects.toThrow('STUDENT_LEFT_GROUP');
    expect(notify).not.toHaveBeenCalled();
  });
  it('does not turn notification failure into a failed review', async () => {
    ...
    try {
      ...
      await expect(mentorReviewService.submit(item, true, '')).resolves.toBeUndefined();
